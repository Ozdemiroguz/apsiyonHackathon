...
final _isSignInProvider = FutureProvider.autoDispose(
  (ref) => ref.watch(authRepositoryProvider).isSignIn(),
);

final _getJobProvider = FutureProvider.autoDispose(
  (ref) => ref.watch(authRepositoryProvider).getUserJob(),
);
      ...
      () {
        ...
        Future.delayed(const Duration(seconds: 1), () async {
          if (await ref.watch(_isSignInProvider.future)) {
            if (context.mounted) {
              final String job = await ref.watch(_getJobProvider.future);

              if (job == 'Kurye') {
                print('Kurye');
                return context.replaceRoute(const MainKuryeRoute());
              } else if (job == 'No User') {
                print('No User');
                return context.replaceRoute(const LoginRoute());
              } else {
                print('Job: $job');
                return context.replaceRoute(const JobMainRoute());
              }
            }
          } else {
            if (context.mounted) {
